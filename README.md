...
每次从数据集中取出 1 组数据，然后放回（Sampling with replacement），重复这个过程 _N_ 次，作为一组数据进行训练 <i>g</i><sub>t</sub>，重复这个过程很多次就可以模拟产生无限多的数据，用这个过程近似理想中的情况。
...
想象一下在实际生活中优化“模型”的过程，以教小孩子区分苹果为例：

1. 找一些苹果和不是苹果的照片
2. 提出一个模型（苹果是圆形的）
3. 根据这个模型做出判断，找出那些对和不对的地方（橘子也是圆形的），把那些不对的部分 **强调** 一下
4. 提出一个新的不一样的模型
5. 重复这个过程
6. 最后整合这些所有的模型
...
有两种情况这个算法会 “被迫” 停下来：
1. 子树中所有的 y 都一样，不需要分割子树
2. 子树中所有的 x 都一样，无法分割子树

这样的到的树就叫做 Fully-grown tree.

### 决策树的问题和特点

Fully-grown tree 显然不一定是好的，比如当所有的数据都不一样的时候，<i>E</i><sub>in</sub> 是 0， 这就是 overfit，这是因为最下面的子树数据量太少了（只有 1 个）。

因此，我们需要 Regularization。比如我们计算树叶的数量 &Omega;(G) ，我们希望在树叶很少的时候 <i>E</i><sub>in</sub> 也能很小。这种就叫做修剪 (pruned)。

所以我们的问题就变成了对于所有可能的树 G 最小化 <i>E</i><sub>in</sub>(G) + &lambda;&Omega;(G)。

但是所有可能的树太多了，通常在实际上我们可以从 Fully-grown tree 开始，一个一个去掉叶子节点（合并），检查 <i>E</i><sub>in</sub> ，找到使 <i>E</i><sub>in</sub> 最小的那个叶子节点。

这样不断循环下去之后，得到的树的集合就是我们需要考虑的树的集合，再计算 <i>E</i><sub>in</sub>(G) + &lambda;&Omega;(G)。

---

除此以外，决策树还有一个特点，就是在处理分类问题的时候，Decision stump 就不再适用了。
代替的算法叫做 Decision subset：穷举所有可能的子集来做分割子树，这样一来就可以比较轻易的处理分类问题。

---

考虑一个真实的情况，当人觉决策（决策树）的时候，输入数据有缺失的时候怎么办？
通常我们会用一些和这个缺失数据类似的数据来代替这个缺失的数据进行分类（决策）。

因此，决策树也有这个特点，可以很好的处理缺失数据。可以在训练的时候找到一些分割子树时的替代条件，并保存起来。在测试或者真实运用的时候，就可以用替代条件来处理。

因为决策树是一种类似人类决策的算法，因此很多人类的行为都可以用来启发决策树算法。
...
`Bagging` 利用 Bootstrap 产生一系列不同的数据，然后训练一个模型，最后综合一系列模型的结果，通过平均 / 投票的方式，可以减少模型的 Variance；
...
这个算法的好处是可以平行（Bagging）+有效（决策树）的计算，同时也继承了决策树的所有好处，同时回避了决策树（尤其是 Fully-grown tree）不稳定（过拟合）的缺点。

---

除了随机生成数据以外，还可以在每次分支子树的时候都选取不同的特征来作为分支规则，相当于把数据向低维进行了 **投影**，通常情况下选取的特征会很少。每次分支都做这样的随机投影，进一步增加了随机性，会得到更多不一样的决策树。
